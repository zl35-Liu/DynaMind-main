...
text_path = '/path/to/DynaMind-main/data/text_embs/text_embeddings.npy'
...
my_model_path = "/path/to/DynaMind-main/outputs/DGVR/checkpoints/tuned/dv16/copy6/10"
...
inv_latent = torch.load('/path/to/DynaMind-main/data/ddim_invs/dv16/ddim_inv_latents_16_first200_loadtextemb.pt',map_location='cpu')
...
for i in range(200):
    ...
    if woSeq2Seq:
        ...
        video = pipe(model, textdata[i:i+1,...].to(dtype=pipe.unet.dtype, device=pipe.device),negative_eeg=negative, latents=inv_latent[i:i+1,...].to(dtype=pipe.unet.dtype, device=pipe.device), video_length=16, height=288, width=512, num_inference_steps=50, guidance_scale=12.5).videos
        savename = ('10')
    save_videos_grid(video, f"./outputs/reconstruction/dv16/{savename}/{i}.gif")



"""
Experiment log for inference variants, covering prompt-conditioned DDIM embeddings, random inverted latents, checkpoint epochs, color artifacts, continuity, and model-selection observations.
"""